...
function Updatestatus
{
	...
	if ($Status -eq 0)
	{
		...
		#Output to Logfile
		$LogDefaultToAuto -f $Logdate | Out-File -filepath $logfile -append 
	}
	if ($Status -eq 2)
	{
...
function BtnOKClick($object)
{
	...
	if ($RadioAutomatically.Checked -eq $True)
	{
		...
		#Output to Logfile
		$TextStrings.LogCommandExecuted -f $Logdate, "Cscript %windir%\system32\SCregEdit.wsf /AU 4" | Out-File -filepath $logfile -append
		$TextStrings.LogCommandExecuted -f $Logdate, ("New-ItemProperty  -path " + $path + " -name Update Status -value " + $value + " -force") | Out-File -filepath $logfile -append 
	...
	if ($RadioWSUS.Checked -eq $True)
	{
		...
		#Output to Logfile
		$TextStrings.LogCommandExecuted -f $Logdate, ("New-Item -path " + $path + " -name WindowsUpdate") | Out-File -filepath $logfile -append

		$path = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate"
		New-Item -path $path -name "AU"
		#Output to Logfile
		$TextStrings.LogCommandExecuted -f $Logdate, ("New-Item -path " + $path + " -name AU") | Out-File -filepath $logfile -append

		$path = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU"
		$value = " 0 "
		New-ItemProperty -path $path -name "NoAutoUpdate" -value $value -force
		#Output to Logfile
		$TextStrings.LogCommandExecuted -f $Logdate, ("New-ItemProperty  -path " + $path + " -name NoAutoUpdate -value " + $value + " -force") | Out-File -filepath $logfile -append

		$path = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU"
		$value = " 1 "
		New-ItemProperty -path $path -name "UseWUServer" -value $value -force
		#Output to Logfile
		$TextStrings.LogCommandExecuted -f $Logdate, ("New-ItemProperty  -path " + $path + " -name UseWUServer -value " + $value + " -force") | Out-File -filepath $logfile -append

		$path = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU"
		$value = " 1 "
		New-ItemProperty -path $path -name "NoAutoRebootWithLoggedOnUsers" -propertytype DWord -value $value
		#Output to Logfile
		$TextStrings.LogCommandExecuted -f $Logdate, ("New-ItemProperty  -path " + $path + " -name NoAutoRebootWithLoggedOnUsers -value " + $value + " -force") | Out-File -filepath $logfile -append

		$path = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate"
		$value = $textboxURLPath.text
		New-ItemProperty -path $path -name "WUServer" -value $value -force
		#Output to Logfile
		$TextStrings.LogCommandExecuted -f $Logdate, ("New-ItemProperty  -path " + $path + " -name WUServer -value " + $value + " -force") | Out-File -filepath $logfile -append

		$path = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate"
		$value = $textboxURLPath.text
		New-ItemProperty -path $path -name "WUStatusServer" -value $value -force
		#Output to Logfile
		$TextStrings.LogCommandExecuted -f $Logdate, ("New-ItemProperty  -path " + $path + " -name WUStatusServer -value " + $value + " -force") | Out-File -filepath $logfile -append

		$path = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update"
		$value = " 2 "
		Set-ItemProperty -path $path -name "AUState" -value $value -force
		#Output to Logfile
		$Logdate + " Command Executed: New-ItemProperty  -path " + $path + " -name AUState -value " + $value + " -force" | Out-File -filepath $logfile -append
										
		$path = "HKLM:\SOFTWARE\Microsoft\Core Configurator"
		$value = " 2 "
		New-ItemProperty -path $path -name "Update Status" -propertytype DWord -value $value -force
		#Output to Logfile
		$Logdate + " Command Executed: New-ItemProperty  -path " + $path + " -name Update Status -value " + $value + " -force" | Out-File -filepath $logfile -append
	...
	if ($RadioNever.Checked -eq $True)
	{
		...
		#Output to Logfile
		$TextStrings.LogCommandExecuted -f $Logdate, ("New-ItemProperty  -path " + $path + " -name Update Status -value " + $value + " -force") | Out-File -filepath $logfile -append
		$TextStrings.LogCommandExecuted -f $Logdate, ("Remove-Item HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate-force -recurse") | Out-File -filepath $logfile -append
	...
	foreach ($update in $txtboxlistupdates.CheckedItems)
	{
		...
		for ($i = 0; $i -lt $results; $i++)
		{
			...
			if (($updates.title.ToUpper()) -eq ($update.ToUpper()))
			{
				...
				$TextStrings.LogCommandExecuted -f $Logdate, ($updates.title + " downloaded.") | Out-File -filepath $logfile -append
				$updatesToDownload.Add($updates) | Out-Null
