...
function StartService ( $object )
{
	...
	#Output to Logfile
	$TextStrings.LogServiceStarted -f $Logdate, $DataGridView1.CurrentCell.Value | Out-File -filepath $logfile -append
...
function RestartService($object)
{
	...
	#Output to Logfile
	$LogServiceRestarted -f $Logdate, $DataGridView1.CurrentCell.Value | Out-File -filepath $logfile -append
...
function StopService($object)
{
	...
	#Output to Logfile
	$LogServiceStopped -f $Logdate, $DataGridView1.CurrentCell.Value | Out-File -filepath $logfile -append
