...
function OK{
	if ($ChkboxEnable.Checked -eq $True)
	{
		...
		$TextStrings.LogRuleGroupChanged -f $Logdate, $TextStrings.WindowsRemoteManagement, $TextStrings.Enabled | Out-File -FilePath $logfile -Append
	}
	if ($ChkboxDisable.Checked -eq $True)
	{
		...
		$TextStrings.LogRuleGroupChanged -f $Logdate, $TextStrings.WindowsRemoteManagement, $TextStrings.Disabled | Out-File -FilePath $logfile -Append
	}	
	Get-Status
	$WinRMform.Update()
}
