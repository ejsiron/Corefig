...
function BtnOKClick($object)
{
	if ($chkboxOn.checked -eq $true)
	{
		...
		$lblStatus.text = "On"
		#Output to Logfile
		$Logdate + " Command Executed: New-ItemProperty -path HKCU:\Control Panel\Desktop -name ScreenSaveActive -value 1 -force"  | Out-File -filepath $logfile -append
		$Logdate + " Command Executed: New-ItemProperty -path HKCU:\Control Panel\Desktop -name ScreenSaveTimeOut -value " + $cmbScreenTime.selecteditem + " -force" | Out-File -filepath $logfile -append
	}
	if ($chkboxOff.checked -eq $true)
	{
		...
		$lblStatus.text = "Off"
		#Output to Logfile
		$Logdate + " Command Executed: New-ItemProperty -path HKCU:\Control Panel\Desktop -name ScreenSaveActive -value 0 -force" | Out-File -filepath $logfile -append
	...
	#Output to Logfile
	$Logdate + " Command Executed: New-ItemProperty -path " + $parentKey.PSpath + "-name DefaultSettings.XResolution -propertytype DWord -value " + $xvalue + " -force" | Out-File -filepath $logfile -append
	$Logdate + " Command Executed: New-ItemProperty -path " + $parentKey.PSpath + "-name DefaultSettings.YResolution -propertytype DWord -value " + $yvalue + " -force" | Out-File -filepath $logfile -append
	$Logdate + " Command Executed: New-ItemProperty -path " + $parentKey.PSpath + "-name DefaultSettings.BitsPerPel -propertytype DWord -value " + $bitvalue + " -force" | Out-File -filepath $logfile -append
}
